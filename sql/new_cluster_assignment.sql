...
patent_cluster_weights AS (
  SELECT DISTINCT
    ...
    weight
  FROM staging_patent_clusters.new_hybrid_sts_scaled_weights
  LEFT JOIN staging_patent_clusters.cluster_assignment_staging AS fam ON id = fam.family_id
  LEFT JOIN staging_patent_clusters.cluster_assignment_staging AS ref ON ref_id = ref.family_id
),
