...
CREATE OR REPLACE TABLE patent_clustering_metrics.cluster_modularity_by_cpc_weights AS (
WITH
  ...
  cluster_ref_tab AS (
    SELECT
      ...
      cluster_ref_id,
    IF
      (cluster_id = cluster_ref_id,1,0) AS internal_link,
    ...
    FROM ( /* assign cluster IDs to referencing papers */
      ...
      FROM
        staging_patent_clusters.cpc_weights) r
    INNER JOIN (
      SELECT
        cluster_id,
        family_id AS id
      FROM
        patent_cluster_experiments.patent_cluster_intial_experiement_sts_scaling_20240726_best_clusters_mapped ) c
    USING
      (id)
    INNER JOIN ( /* assign cluster IDs to referenced papers */
      SELECT
        ...
        family_id AS ref_id
      FROM
        patent_cluster_experiments.patent_cluster_intial_experiement_sts_scaling_20240726_best_clusters_mapped ) d
    USING
      (ref_id) ),
