-- Finding the patent assignment level for patents in the new clusters, based on each patent's total number of within-cluster links
-- as well as the fraction of patent links that connect within-cluster.
CREATE OR REPLACE TABLE patent_clustering_metrics.patent_assignment_level_by_weights AS (
  WITH
    -- Finding links that connect patents within the same cluster
    new_within_cluster_links AS (
      SELECT
        ref.family_id AS family_id,
        ref.family_reference AS ref_id,
        c1.cluster_id AS cluster_id
      FROM staging_patent_clusters.combined_weights AS ref
      LEFT JOIN patent_cluster_experiments.patent_cluster_intial_experiement_sts_scaling_20240726_best_clusters_mapped AS c1 ON ref.family_id = c1.family_id
      LEFT JOIN patent_cluster_experiments.patent_cluster_intial_experiement_sts_scaling_20240726_best_clusters_mapped AS c2 ON ref.family_reference = c2.family_id
    ...
    -- Finding relevent metrics for each article
    new_patent_links AS (
      SELECT
       document_id,
        cluster_id,
        ...
        COALESCE(n_references_cluster + n_citations_cluster, n_references_cluster, n_citations_cluster, 0) AS cluster_links,
        COALESCE(n_references_cluster + n_citations_cluster, n_references_cluster, n_citations_cluster, 0)/COALESCE(n_references_total + n_citations_total, n_references_total, n_citations_total, 1) AS frac
      ...
      SELECT
        *,
        CASE
          ...
          -- 3: VERY WELL
          WHEN (cluster_links >=20 AND frac >= 0.1 AND frac < 0.5) OR (cluster_links >= 5 AND cluster_links <= 20 AND frac >= thres1 AND frac < 0.5) THEN 3
          -- 2: DEFENSIBLE
          WHEN frac >= 0.03 AND cluster_links >= 2 AND ((cluster_links < 5 AND frac >= 0.1) OR (frac < 0.1 AND frac >= thres2 AND cluster_links < 10) OR (cluster_links >= 10 AND frac < 0.1) OR (cluster_links >= 5 AND frac >= 0.1 AND cluster_links < 20 AND frac < thres1)) THEN 2
          -- 1: WEAK
