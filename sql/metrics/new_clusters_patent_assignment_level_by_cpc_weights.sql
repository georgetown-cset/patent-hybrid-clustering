...
CREATE OR REPLACE TABLE patent_clustering_metrics.new_clusters_patent_assignment_level_by_cpc_weights AS (
  WITH
  -- Finding links that connect patents within the same cluster
  new_within_cluster_links AS (
    SELECT
      cpc_weights.family_id AS family_id,
      cpc_weights.family_reference AS ref_id,
