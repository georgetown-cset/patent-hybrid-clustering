...
translation_not_needed AS (
  SELECT DISTINCT
    ...
    family_id,
    title,
    metadata_d_p_removed.title_original,
    abstract,
    metadata_d_p_removed.abstract_original
  FROM
    staging_patent_clusters.metadata_d_p_removed
  LEFT JOIN
...
translated AS (
  SELECT DISTINCT
    patent_id,
    family_id,
    translated_patents.title,
    translated_patents.title_original,
    translated_patents.abstract,
    translated_patents.abstract_original
  FROM
    staging_patent_clusters.metadata_d_p_removed
  INNER JOIN
