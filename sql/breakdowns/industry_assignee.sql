...
top10_tab AS (
  SELECT
    cluster_id,
    COUNT(DISTINCT family_id) AS NPF_top10_industry_assignees
  FROM
    assignees
  INNER JOIN (
    SELECT
...
miss_assignee_tab AS (
  SELECT
    cluster_id,
    SUM(miss_org) AS NPF_missing_all_industry_assignees
  FROM (
    SELECT
      cluster_id,
      family_id,
      MIN(
        IF(industry_assignee IS NULL, 1, 0)) AS miss_org
    FROM
      assignees
    GROUP BY
      family_id,
