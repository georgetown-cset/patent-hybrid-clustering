-- Find percentage robotics and robotics subcategories by cluster
...
robotics_tab AS (
  SELECT
    ...
    CAST(Robot_Type__Delivery AS INT64) AS Robot_Type__Delivery,
    CAST(Robot_Type__Disaster_Response AS INT64) AS Robot_Type__Disaster_Response,
    CAST(Robot_Type__Drones AS INT64) AS Robot_Type__Drones,
    ...
    CAST(Robot_Type__Research AS INT64) AS Robot_Type__Research,
    CAST(Robot_Type__Service AS INT64) AS Robot_Type__Service,
    CAST(Robot_Type__Social AS INT64) AS Robot_Type__Social,
...
merged AS (
  SELECT DISTINCT
    ...
    Robot_Type__Delivery,
    Robot_Type__Disaster_Response,
    Robot_Type__Drones,
...
SELECT
  ...
  SUM(Robot_Type__Delivery) / NULLIF(COUNT(family_id), 0) AS Robot_Type__Delivery_pred,
  SUM(Robot_Type__Disaster_Response) / NULLIF(COUNT(family_id), 0) AS Robot_Type__Disaster_Response_pred,
  SUM(Robot_Type__Drones) / NULLIF(COUNT(family_id), 0) AS Robot_Type__Drones_pred,
  ...
  SUM(Robot_Type__Research) / NULLIF(COUNT(family_id), 0) AS Robot_Type__Research_pred,
  SUM(Robot_Type__Service) / NULLIF(COUNT(family_id), 0) AS Robot_Type__Service_pred,
  SUM(Robot_Type__Social) / NULLIF(COUNT(family_id), 0) AS Robot_Type__Social_pred,
