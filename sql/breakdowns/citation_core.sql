-- Find "core" papers by cluster where core stat is defined based on cluster citation links
...
-- get citations of new patent families; keep references last 10 years
new_cit AS (
  SELECT
    family_id AS id,
    ref_id
  FROM
    cit
  -- Convert references from individual patents to families
  LEFT JOIN (
    SELECT
      patent_id,
