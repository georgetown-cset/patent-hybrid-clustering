...
percentages as (
  SELECT
  cluster_id,
  SUM(IF(category = "agriculture_forestry_fishing", 1, 0))/NULLIF(COUNT(family_id), 0) AS agriculture_forestry_fishing,
  SUM(IF(category = "wholesale_retail_motor_vehicle_repair", 1, 0))/NULLIF(COUNT(family_id), 0) AS wholesale_retail_motor_vehicle_repair,
  ...
  SUM(IF(category = "accommodation_and_food_service", 1, 0))/NULLIF(COUNT(family_id), 0) AS accommodation_and_food_service,
  SUM(IF(category = "real_estate", 1, 0))/NULLIF(COUNT(family_id), 0) AS real_estate,
  SUM(IF(category = "professional_scientific_and_technical_activities", 1, 0))/NULLIF(COUNT(family_id), 0) AS professional_scientific_and_technical_activities,
  SUM(IF(category = "human_health_and_social_work_activities", 1, 0))/NULLIF(COUNT(family_id), 0) AS human_health_and_social_work_activities,
  SUM(IF(category = "information_and_communication", 1, 0))/NULLIF(COUNT(family_id), 0) AS information_and_communication,
  ...
  SUM(IF(category = "electrical_equipment_manufacturing", 1, 0))/NULLIF(COUNT(family_id), 0) AS electrical_equipment_manufacturing,
  SUM(IF(category = "electricity_gas_steam_ac_supply", 1, 0))/NULLIF(COUNT(family_id), 0) AS electricity_gas_steam_ac_supply,
  SUM(IF(category = "arts_entertainment_recreation", 1, 0))/NULLIF(COUNT(family_id), 0) AS arts_entertainment_recreation,
  ...
  SUM(IF(category = "diagnosis_medical_equipment", 1, 0))/NULLIF(COUNT(family_id), 0) AS diagnosis_medical_equipment,
  ...
  SUM(IF(category = "miscellaneous_manufacturing", 1, 0))/NULLIF(COUNT(family_id), 0) AS miscellaneous_manufacturing,
  SUM(IF(category = "layered_products_manufacturing", 1, 0))/NULLIF(COUNT(family_id), 0) AS layered_products_manufacturing,
  ...
  SUM(IF(category = "measuring_and_control_equipment", 1, 0))/NULLIF(COUNT(family_id), 0) AS measuring_and_control_equipment,
  ...
  SUM(IF(category = "semiconductors_and_electronics", 1, 0))/NULLIF(COUNT(family_id), 0) AS semiconductors_and_electronics,
  SUM(IF(category = "telecommunications", 1, 0))/NULLIF(COUNT(family_id), 0) AS telecommunications,
  SUM(IF(category = "other_vehicle_transportation", 1, 0))/NULLIF(COUNT(family_id), 0) AS other_vehicle_transportation,
  SUM(IF(category = "road_vehicle_transportation", 1, 0))/NULLIF(COUNT(family_id), 0) AS road_vehicle_transportation,
  SUM(IF(category = "water_supply_sewage_waste_mgmt_remediation", 1, 0))/NULLIF(COUNT(family_id), 0) AS water_supply_sewage_waste_mgmt_remediation,
FROM
  merged
GROUP BY
  cluster_id
