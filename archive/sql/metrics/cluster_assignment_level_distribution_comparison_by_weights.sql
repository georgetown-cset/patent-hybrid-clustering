...
CREATE OR REPLACE TABLE patent_clustering_metrics.cluster_assignment_level_distribution_comparison_by_weights AS (
  WITH
  ...
  old_cluster_assignment_levels AS (
    ...
    FROM patent_clustering_metrics.old_clusters_patent_assignment_level_by_weights
    GROUP BY cluster_id
  ),
  ...
  new_cluster_assignment_levels AS (
    SELECT
      cluster_id,
      AVG(assignment_level) AS cluster_assignment_level
    FROM patent_clustering_metrics.new_clusters_patent_assignment_level_by_weights
    GROUP BY cluster_id
  ),
