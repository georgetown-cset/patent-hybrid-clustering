...
CREATE OR REPLACE TABLE patent_clustering_metrics.new_clusters_patent_assignment_level AS (
  WITH
  -- Finding links that connect patents within the same cluster
  new_within_cluster_links AS (
    SELECT
      family_references.family_id AS family_id,
      family_references.family_reference AS ref_id,
