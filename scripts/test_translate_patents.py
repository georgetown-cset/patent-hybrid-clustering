...
class TestTranslate(unittest.TestCase):
    ...
    def test_translate_text(self):
        translate = translate_patents.Translator("../data/test_translated.jsonl")
        text = "Fremgangsmåte for konservering av fullmodne poteter og " \
               "andre rotfrukter ved hjelp av dypfrysning."
        result = translate.translate_text(text)
        self.assertNotEqual(text, result)
        self.assertEqual(result, "Method for the preservation of fully ripe potatoes "
    ...
    def test_remove_bad_chars(self):
        translate = translate_patents.Translator("../data/test_translated.jsonl")
        bad_text = "Fremgangsmåte \x96for konservering av fullmodne poteter og " \
               "andre rotfrukter ved hjelp av dypfrysning."
        fixed_text = "Fremgangsmåte for konservering av fullmodne poteter og " \
               "andre rotfrukter ved hjelp av dypfrysning."
    ...
    def test_validate_translation(self):
        translate = translate_patents.Translator("../data/test_translated.jsonl")
        text = "Method for the preservation of fully ripe potatoes " \
               "and other root vegetables by means of deep freezing."
        self.assertTrue(translate.validate_translation(text))
        non_english_text = "Fremgangsmåte for konservering av fullmodne poteter og " \
               "andre rotfrukter ved hjelp av dypfrysning."
