...
1. An Airflow pipeline [`patent_hybrid_clustering`](https://d7c5f2369fea46beb8ec7a3532550598-dot-us-east1.composer.googleusercontent.com/dags/patent_hybrid_clustering/grid?search=patent_hybrid_clustering) and supporting code to run updates of the patent clustering
2. Supporting code for an Airflow pipeline [`mop_bq_to_bq_metadata`](https://d7c5f2369fea46beb8ec7a3532550598-dot-us-east1.composer.googleusercontent.com/dags/mop_bq_to_bq_metadata/grid) to transform the patent clustering data into production-ready Map of Patents data capable of being transferred to the Cloud SQL instance for use by Map of Patents. This DAG is auto generated by the [`sql_updater_with_checks_dag.py`](https://github.com/georgetown-cset/airflow_sql_sequence_templates/blob/main/dags/sql_updater_with_checks_dag.py) template in the `airflow_sql_sequence_templates` repo. It reads from [`mop_bq_to_bq_metadata.json`](config/mop_bq_to_bq_metadata.json) config file and runs the [SQL files](sql/map_of_patents/) and [corresponding checks](sql/map_of_patents_checks/) to populate the `staging_map_of_patents` BQ dataset.
